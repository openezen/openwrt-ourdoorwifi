...
static inline void *__dma_zalloc_coherent(struct device *dev, size_t size, dma_addr_t *dma_handle, gfp_t flag)
	...
	OUT PNDIS_PHYSICAL_ADDRESS	phy_addr)
{
	...
	*VirtualAddress = (PVOID)__dma_zalloc_coherent(pdev, sizeof(char) * Length, &DmaAddr, GFP_KERNEL);
