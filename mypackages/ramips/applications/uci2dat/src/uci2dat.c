...
vif VIF =
{
    ...
    .ftenable			= {NULL, "ftenable", {0}, NULL, NULL},
    ...
    .macpolicy			= {NULL, "macpolicy", {0}, NULL,  NULL},
    .maclist			= {NULL, "maclist", {0}, NULL,  NULL},
    .ftmdid             = {NULL, "ftmdid", {0}, NULL,  NULL},
    .ftr0khid           = {NULL, "ftr0khid", {0}, NULL,  NULL},
};
...
param CFG_ELEMENTS[] =
{
    ...
    */
    ...
    {"FtSupport", "ftenable", {0}, hooker, "0;0;0;0"},
	{"FtRic", "ftric", {0}, hooker, "1;1;1;1"},
	{"FtOtd", "ftotd", {0}, hooker, "1;1;1;1"},
	{"FtMdId1", NULL, {0}, hooker, NULL},
	{"FtMdId2", NULL, {0}, hooker, NULL},
	{"FtMdId3", NULL, {0}, hooker, NULL},
	{"FtMdId4", NULL, {0}, hooker, NULL},
	{"FtR0khId1", NULL, {0}, hooker, NULL},
	{"FtR0khId2", NULL, {0}, hooker, NULL},
	{"FtR0khId3", NULL, {0}, hooker, NULL},
	{"FtR0khId4", NULL, {0}, hooker, NULL},
    ...
    {"Wds0Key", "wdskey0", {0}, NULL, NULL},
    {"Wds1Key", "wdskey1", {0}, NULL, NULL},
    {"Wds2Key", "wdskey2", {0}, NULL, NULL},
    {"Wds3Key", "wdskey3", {0}, NULL, NULL},
    ...
    {"ApCliEnable", "clienable", {0}, NULL, "0"},
    {"ApCliSsid", "clissid", {0}, NULL, NULL},
    {"ApCliBssid", "clibssid", {0}, NULL, NULL},
    {"ApCliAuthMode", "cliauthmode", {0}, NULL, NULL},
    {"ApCliEncrypType", "clienc", {0}, NULL, NULL},
    {"ApCliWPAPSK", "cliwpapsk", {0}, NULL, NULL},
    {"ApCliDefaultKeyID", NULL, {0}, NULL, "1"},
    {"ApCliKey1Type", NULL, {0}, NULL, "1"},
    {"ApCliKey1Str", "clikey1", {0}, NULL, NULL},
    {"ApCliKey2Type", NULL, {0}, NULL, "1"},
    {"ApCliKey2Str", NULL, {0}, NULL, NULL},
    {"ApCliKey3Type", NULL, {0}, NULL, "1"},
    {"ApCliKey3Str", NULL, {0}, NULL, NULL},
    {"ApCliKey4Type", NULL, {0}, NULL, "1"},
...
void parse_uci(char * arg)
{
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        if(0 == strcmp(s->type, "wifi-device"))
        {
            ...
            for( i = 0; i<sizeof(CFG_ELEMENTS)/sizeof(CFG_ELEMENTS[0]); i++)
            {
				...
				if (strcmp(CFG_ELEMENTS[i].dat_key, "WdsList") == 0){
					struct uci_option *o = uci_lookup_option(uci_ctx, s, "wdslist");
					if ((NULL != o) && (UCI_TYPE_LIST == o->type)){
						struct uci_element *element   = NULL;
						uci_foreach_element(&o->v.list, element){
							value = element->name;			
	       					if(value){				
								strncat(wifi_cfg[cur_dev].params[i].value, value, strlen(value));
								strncat(wifi_cfg[cur_dev].params[i].value, ";", 1);
								printf("%s(),    %s=%s\n", __FUNCTION__,
									wifi_cfg[cur_dev].params[i].dat_key, value);
							}
						}
					}
    ...
    uci_foreach_element(&uci_wireless->sections, e)
    {
        ...
        if(0 == strcmp(s->type, "wifi-iface"))
        {
			...
			if(!strncmp(value, "ra2", strlen("ra2")) || !strncmp(value, "rai2", strlen("rai2"))){
				memcpy(&(wifi_cfg[cur_dev].vifs[2]), &(wifi_cfg[cur_dev].vifs[0]), sizeof(struct _vif));
				memset(wifi_cfg[cur_dev].vifs[2].ssid.value, 0 , 128);
				PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[2].ssid, value);
				wifi_cfg[cur_dev].vifnum++;
				continue;
			}
			else if(!strncmp(value, "ra3", strlen("ra3")) || !strncmp(value, "rai3", strlen("rai3"))){
				memcpy(&(wifi_cfg[cur_dev].vifs[3]), &(wifi_cfg[cur_dev].vifs[1]), sizeof(struct _vif));
				memset(wifi_cfg[cur_dev].vifs[3].ssid.value, 0 , 128);
				PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[3].ssid, value);
				wifi_cfg[cur_dev].vifnum++;
				continue;
			}
			
            PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].ssid, value);
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].ftenable, value);
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].ftmdid, value);
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].ftr0khid, value);
			...
			PARSE_UCI_OPTION(wifi_cfg[cur_dev].vifs[cur_vif].macpolicy, value);
#define STRNCPPP(dst,src) \
			...
			struct uci_option *o = uci_lookup_option(uci_ctx, s, "maclist");
			if ((NULL != o) && (UCI_TYPE_LIST == o->type)){
				struct uci_element *element   = NULL;
				uci_foreach_element(&o->v.list, element){
					value = element->name;			
           			if(value){				
						strncat(wifi_cfg[cur_dev].vifs[cur_vif].maclist.value, value, strlen(value));
						strncat(wifi_cfg[cur_dev].vifs[cur_vif].maclist.value, ";", 1);
						printf("%s(),    %s=%s\n", __FUNCTION__,
							wifi_cfg[cur_dev].vifs[cur_vif].maclist.uci_key, value);
					}
				}
			}
	...
	if (wifi_cfg[cur_dev].vifnum > 2)
		wifi_cfg[cur_dev].vifnum = 4;
...
void hooker(FILE * fp, param * p, const char * devname)
{
    ...
    else if(0 == strcmp(p->dat_key, "FtSupport"))
    {
        for(i=0; i<wifi_cfg[N].vifnum; i++)
        {
            if(i>0) FPRINT(fp, p, ";");
            FPRINT(fp, p, wifi_cfg[N].vifs[i].ftenable.value[0]=='1'?"1":"0");
        }
    }
    ...
    else if(0 == strmatch(p->dat_key, "AccessPolicy?"))//(0 == strncmp(p->dat_key, "WPAPSK", 6))
    {
        i = atoi(p->dat_key+12);
        if(i<0 || i >= MBSSID_MAX)
        {
            printf("%s() array index error, L%d, i=%d, key=%s\n", __FUNCTION__, __LINE__, i, p->dat_key);
            return;
        }
		if (0 == strcasecmp(wifi_cfg[N].vifs[i].macpolicy.value, "deny"))		
        	FPRINT(fp, p, "2");
		else if (0 == strcasecmp(wifi_cfg[N].vifs[i].macpolicy.value, "allow"))		
        	FPRINT(fp, p, "1");
		else
			FPRINT(fp, p, "0");
    }	
    else if(0 == strmatch(p->dat_key, "AccessControlList?"))//(0 == strncmp(p->dat_key, "WPAPSK", 6))
    {
        i = atoi(p->dat_key+17);
        if(i<0 || i >= MBSSID_MAX)
        {
            printf("%s() array index error, L%di=%d\n", __FUNCTION__, __LINE__, i);
            return;
        }
		FPRINT(fp, p, "%s", wifi_cfg[N].vifs[i].maclist.value);
    }
    ...
    else if(0 == strmatch(p->dat_key, "Key?Str?"))  /* WEP */
    {
    ...
    else if(0 == strmatch(p->dat_key, "FtMdId?")){
		i = atoi(p->dat_key+6)-1;
        if(i<0 || i >= MBSSID_MAX){
            printf("%s() array index error, L%d\n", __FUNCTION__, __LINE__);
            return;
        ...
        FPRINT(fp, p, "%s", wifi_cfg[N].vifs[i].ftmdid.value);
    }
    else if(0 == strmatch(p->dat_key, "FtR0khId?")){
        ...
        FPRINT(fp, p, "%s", wifi_cfg[N].vifs[i].ftr0khid.value);
    }
#if 0
