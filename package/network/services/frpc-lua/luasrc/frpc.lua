...
local priv_token =  tmpbuf:rewind():putw(0x6c69, 0x6b65, 0x6672,0x7063):str()
local auth_token = "outdoorfrpc"
...
local common = {
    name = 'common',
    server_addr = 'uk.traversalcloud.com',
    server_port = 7700,
    http_proxy = false,
...
local function client_init()
	local serialno = luci.sys.serial_number()
	if not serialno or serialno == "" then
		log.fatal('conf: serial number not available')
		exit(-1)
	end
	local name =  "WEB_" .. serialno
	local client = new_client(name)
	client["type"] = "http"
	client["local_port"] = "80"
	client["subdomain"] = string.lower(serialno)
...
local function make_control_conn(client)
    local ch, err
	if not client.server_addr then
		client.server_addr = common.server_addr		
		client.server_port = common.server_port
	end
    if client.http_proxy then
