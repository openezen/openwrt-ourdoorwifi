...
//alert("tablesorterPager");
		this.defaults = {
			// target the pager markup
			container: null,
			enablepagermode:true,
			ishidden:true,
	...
	//alert("my enable");
			...
			//alert("dk="+c.totalPages);
			if ( triggered ) {
			//alert("yuiiooooo123456677");
				$(table).trigger('update');
				//alert(c.size);
				setPageSize(table, c.size, c);
				//alert("yuiioooootttttttt");
		...
		$this.changemyparam = function(enable) {
		alert("change="+enable);
			var c = table.config.pager;
			c.ishidden = enable;
			//c.size=1000;
		...
		//alert("construct");
			return this.each(function() {
				...
				var asd;
				asd = this.config;
				//alert("before="+settings.ishidden);
...
//alert("c.ajaxUrl="+c.ajaxUrl);
//alert(c.enablepagermode);
				if(c.enablepagermode)
				enablePager(table, c, true);
//alert("fxing");
...
$.fn.extend({
	...
	changemyparam: $.tablesorterPager.changemyparam
});
