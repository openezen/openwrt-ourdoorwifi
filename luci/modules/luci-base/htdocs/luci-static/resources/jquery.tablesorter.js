...
//alert("tyui");
			...
			//alert($(table).find('thead:eq(0), tfoot').children('th')[0].width());
	...
	/*		
			...
			function fixColumnWidth(table) {
				...
				if (table.config.widthFixed && $(table).find('colgroup').length === 0) {
					...
					//alert(parseInt(($(this).width()/overallWidth)*1000, 10)/10 + '%');
						colgroup.append($('<col>').css('width', parseInt(($(this).width()/overallWidth)*1000, 10)/10 + '%'));
					});
					$(table).prepend(colgroup);
				}
...
//alert("aaaa66");			
			c.$headers
				...
				.bind('mousedown.tablesorter mouseup.tablesorter sort.tablesorter keypress.tablesorter', function(e, external) {
					...
					//alert("aaaa88");
					//alert(/TH|TD/.test(this.tagName));
					// jQuery v1.2.6 doesn't have closest()
					var $cell = /TH|TD|th|td/.test(this.tagName) ? $(this) : $(this).parents('th, td').filter(':first'), cell = $cell[0];
					//var $cell = true ? $(this) : $(this).parents('th, td').filter(':first'), cell = $cell[0];
					//alert($this);
					//alert("3="+cell.sortDisabled);
