	...
	for iface in ifaces:gmatch("[%w%.%-_]+") do
		...
		if device then
			...
			if data.proto == "qmi" and data.is_up then
				...
				qmiinfo = string.format((rv and rv.plmn or "UNKNOWN") .. "  " .. (string.upper(rv and rv.isptype or "UNKNOWN") ) .. "  RSRP:" .. (rv and rv.rsrp or "No signal"))
				data.qmiinfo = qmiinfo
			end

			if data.proto == "3g" then
				data.ifname = data.ifname:gsub("3g", "4g")
				data.name = data.name:gsub("3g", "4g")
			end
