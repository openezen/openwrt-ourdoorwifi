...
local version,_ = luci.sys.get_sys_version()
...
mac.cfgvalue = function(self, section)
	local macaddr=luci.util.exec("cat /sys/class/net/ra1/address")
	if macaddr then
		return macaddr
	else
		return "none"
...
if std_name ~= "S3A"  and nixio.fs.access("/usr/bin/iconv")  then
	...
	gbk_enable = s:option(Flag, "gbk_enable", translate("Enable GBK SSID for Windows computer"), translate("If not selected only support iOS and Android devices."))
	gbk_enable.rmempty = false
end
...
function m.on_parse(map)
	local enc = encrypt:formvalue("guest")
	if enc == "wep" then
	...
	if have_wifi_5G and have_wifi_5G ~= '0' then
		local enc5g = encrypt:formvalue("guest_5g")
		if enc5g == "wep" then
...
mac5g.cfgvalue = function(self, section)
	local macaddr=luci.util.exec("cat /sys/class/net/rai1/address")
	if macaddr then
		return macaddr
	else
		return "none"
